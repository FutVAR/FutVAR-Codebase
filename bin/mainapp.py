...
def run_radar(source_video_path: str, device: str) -> Iterator[np.ndarray]:
    ...
    for frame in frame_generator:
        ...
        players.class_id = team_classifier.predict(players_crops)

        goalkeepers_team_id = resolve_goalkeepers_team_id_radar(players, goalkeepers)
...
def run_voronoi(source_video_path: str, device: str) -> Iterator[np.ndarray]:
    ...
    if len(crops) > 0:
        ...
        raise ValueError("No player detections found in video frames, cannot train team classifier.")
    ...
    for frame in frame_generator:
        ...
        all_detections = detections[detections.class_id != BALL_CLASS_ID]
        all_detections = all_detections.with_nms(threshold=0.5, class_agnostic=True) if len(all_detections) > 0 else sv.Detections([])

        # Ensure there are detections before updating the tracker
        if len(all_detections) > 0:
            all_detections = tracker.update_with_detections(detections=all_detections)
        else:
            all_detections = sv.Detections([])  # Empty detections if no detections

        goalkeepers_detections = all_detections[all_detections.class_id == GOALKEEPER_CLASS_ID]
        players_detections = all_detections[all_detections.class_id == PLAYER_CLASS_ID]
        referees_detections = all_detections[all_detections.class_id == REFEREE_CLASS_ID]

        # Check for player detections before proceeding with team assignment
        if len(players_detections) > 0:
            players_crops = [sv.crop_image(frame, xyxy) for xyxy in players_detections.xyxy]
            players_detections.class_id = team_classifier.predict(players_crops)
        else:
            players_detections.class_id = np.array([])  # Empty array to avoid issues

        # Safeguard for goalkeepers team assignment
        if len(goalkeepers_detections) > 0:
            ...
            goalkeepers_detections.class_id = np.array([])  # Empty array to avoid issues
        
        # Ensure referees_detections is not empty
        if len(referees_detections) > 0:
            referees_detections.class_id -= 1
        else:
            referees_detections.class_id = np.array([])  # Empty array to avoid issues
        ...
        # Merge detections after processing
        ...
        # Ensure Voronoi rendering only if there are player detections
        if len(players_detections) > 0 and len(keypoints) > 0:
            voronoi = render_voronoi(players_detections, keypoints)
...
def run_segmentation(source_video_path: str, device: str) -> Iterator[np.ndarray]:
    player_detection_model = YOLO(PLAYER_DETECTION_MODEL_PATH).to(device=device)
...
def process_video(source_video_path: str, target_video_path: str, device: str, mode: Mode) -> str:
    """
    Processes the video based on the selected mode and saves the output.

    Args:
        source_video_path (str): Path to the input video.
        target_video_path (str): Path to save the output video.
    ...
    elif mode == Mode.HEAT_MAP:
        frame_generator = run_heatmap(
            source_video_path=source_video_path, device=device)

    elif mode == Mode.ACTION_RECOGNITION:
        frame_generator = run_player_detection(
            source_video_path=source_video_path, device=device)
    ...
    video_info = sv.VideoInfo.from_video_path(source_video_path)
    with sv.VideoSink(target_video_path, video_info, codec="h264") as sink:
        for frame in frame_generator:
...
def main(source_video_path: str, target_video_path: str, device: str, mode: Mode) -> None:
    ...
    """
    output_video_path = process_video(source_video_path, target_video_path, device, mode)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mode', type=str, choices=[m.value for m in Mode], default=Mode.PLAYER_DETECTION.value) # Changed type to str and choices
    args = parser.parse_args()
    main(source_video_path=args.source_video_path,
        target_video_path=args.target_video_path,
        device=args.device,
        mode=Mode(args.mode) # Convert string back to Mode enum here
    )